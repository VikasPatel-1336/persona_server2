...
const contactdetails = async (req, res) => {
 ...
 const getStudyMaterialDetails = async (req, res) => {
  try {
    const details = await StudyMaterial.find();
    res.status(200).json({ details });
  } catch (err) {
    res.status(400).json({
      status: "fail",
...
module.exports = { getCourseDetails, contactdetails,getQuizDetails,getStudyMaterialDetails};
